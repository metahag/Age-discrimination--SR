...
\indent Our first meta-analysis encompassed all within-subject correspondence studies measuring the hiring disparities between our comparator group and 40 to 49 year old applicants. Via a random-effects meta-analysis (k=2), the three included studies revealed an effect of applicant age on the hiring decisions held by the participating recruiters against older applicants, with the odds ratio being `r  random_forty_ws_exp$pred ` (95 % CI [ `r random_forty_ws_exp$ci.lb`, `r random_forty_ws_exp$ci.ub` ], 95% PI [ `r random_forty_ws_exp$pi.lb`, `r random_forty_ws_exp$pi.ub` ], z = `r random_forty_ws$zval `, p = `r round(random_forty_ws$pval, digits = 10)`), which means that on average the odds of older applicant being hired compared to the control applicant were `r round(((1 - random_forty_ws_exp$pred) * 100), digits = 2)`% lower. Total variance was low (mostly due to a low number of included studies; k=2) tau^2 = `r  round(exp(random_forty_ws$tau2), digits = 2) ` (SD of the true effects across studies was tau = `r  round(exp(random_forty_ws$tau2), digits = 2)`), and the heterogeneity was I^2 = `r  random_forty_ws$I2 `%.

\indent Our second meta-analysis was set to include all between-subject correspondence studies measuring the hiring disparities between our comparator group and 40 to 49 year old applicants. Via a random-effects meta-analysis, the three studies included revealed a significant effect of applicant age on the hiring decisions held by the participating recruiters against older applicants, with odds ratio of `r  random_forty_bs_exp$pred ` (95 % CI [ `r random_forty_bs_exp$ci.lb`, `r random_forty_bs_exp$ci.ub` ], 95% PI [ `r random_forty_bs_exp$pi.lb`, `r random_forty_bs_exp$pi.ub` ], z = `r random_forty_bs$zval `, p = `r round(random_forty_bs$pval, digits = 10)`), meaning they had `r round(((1 - random_forty_bs_exp$pred) * 100), digits = 2)`% lower odds of being hired compared to younger applicants. Tau^2 and I^2 were zero, however heterogeneity of studies is difficult to assess with such a small number of included studies (k=3), so this does not imply homogeneity.  

\indent Age category - Fifty 
\indent Our third meta-analysis was set to include all within-subject correspondence studies measuring the hiring disparities between our comparator group and 50 to 59 year old applicants. Via a random-effects meta-analysis, the  study on the matter of age discrimination revealed an effect of applicant age on the hiring decisions held by the participating recruiters against older applicants The overall effect was OR = `r  random_fifty_ws_exp$pred ` (95 % CI [ `r random_fifty_ws_exp$ci.lb`, `r random_fifty_ws_exp$ci.ub` ], 95% PI [ `r random_fifty_ws_exp$pi.lb`, `r random_fifty_ws_exp$pi.ub` ], z = `r random_fifty_ws$zval `, p = `r round(random_fifty_ws$pval, digits = 10)`), with `r round(((1 - random_fifty_ws_exp$pred) * 100), digits = 2)`% lower odds of being hired compared to control. Once again, tau^2 and I^2 values (`r  round(exp(random_fifty_ws$tau2), digits = 2) `; `r  round(random_fifty_ws$I2, digits = 2)`%, resp.) for such a small sample of studies (k=3) do not provide meaningful assessment of heterogeneity.

\indent Our fourth meta-analysis was set to include all between-subject correspondence studies measuring the hiring disparities between our comparator group and 50 to 59 year old applicants. Via a random-effects meta-analysis we found that on average older applicants had a `r round(((1 - random_fifty_bs_exp$pred) * 100), digits = 2)`% lower odds of being invited to an interview/hired than the younger applicant (OR = `r  random_fifty_bs_exp$pred `, 95 % CI [ `r random_fifty_bs_exp$ci.lb`, `r random_fifty_bs_exp$ci.ub` ], 95% PI [ `r random_fifty_bs_exp$pi.lb`, `r random_fifty_bs_exp$pi.ub` ], z = `r random_fifty_bs$zval `, p = `r round(random_fifty_bs$pval, digits = 10)`). As in the former analysis, heterogeneity was not found (tau2 = `r  round(exp(random_fifty_bs$tau2), digits = 2) `; I2 = `r  random_fifty_bs$I2 `%). 


\indent Age category - Sixty
\indent Our fifth meta-analysis was set to include all between-subject correspondence studies measuring the hiring disparities between our comparator group and 60 to 65 year old applicants. Via a random-effects meta-analysis, we found on average that older applicants have a `r round(((1 - random_sixty_bs_exp$pred) * 100), digits = 2)`% lower odds of getting hired campared to younger applicants (OR = `r  random_sixty_bs_exp$pred `, 95 % CI [ `r random_sixty_bs_exp$ci.lb`, `r random_sixty_bs_exp$ci.ub` ], 95% PI [ `r random_sixty_bs_exp$pi.lb`, `r random_sixty_bs_exp$pi.ub` ], z = `r random_sixty_bs$zval `, p = `r round(random_sixty_bs$pval, digits = 10)`). Total variance was low (tau^2 = `r  exp(random_sixty_bs$tau2) `, with the SD of the true effects across studies, tau = `r  round(exp(random_sixty_bs$tau2), digits = 2)`), with `r round(random_sixty_bs$I2, digits = 0) `% of it being heterogeneity of true effects (I^2 = `r  random_sixty_bs$I2 `). 


\indent Age category - Over Sixtyfive
\indent The final meta-analysis included all between-subject correspondence studies measuring the hiring disparities between our comparator group and applicants over 65 years of age. Via a random-effects meta-analysis, we found that on average, older applicants have a `r round(((1 - random_above_sixtyfive_bs_exp$pred) * 100), digits = 2)`% lower odds of being hired compared to control (OR = `r  random_above_sixtyfive_bs_exp$pred `, 95 % CI [ `r random_above_sixtyfive_bs_exp$ci.lb`, `r random_above_sixtyfive_bs_exp$ci.ub` ], 95% PI [ `r random_above_sixtyfive_bs_exp$pi.lb`, `r random_above_sixtyfive_bs_exp$pi.ub` ], z = `r random_above_sixtyfive_bs$zval `, p = `r round(random_above_sixtyfive_bs$pval, digits = 10)`). Total variance was relatively low (tau^2 = `r  random_above_sixtyfive_bs$tau2 ` and the SD of the true effects across studies, tau = `r  round(exp(random_above_sixtyfive_bs$tau2), digits = 2)`), with effect heterogeneity accounting for mostly all of that variance (I^2 = `r  random_above_sixtyfive_bs$I2 `). 
...
\indent Based on the aforementioned risk of bias assessments, the funnel plots presented below and the p-curve (Figure 4; Results.Rmd at https://osf.io/zqxga/), we conclude to find a moderate certainty of evidence. In general we find narrow confidence intervals (Figure 2; Figure 3) and prediction intervals, meaning our average true effect and the deviation of found effects show good precision of evidence. Furthermore, we conducted the leave-one-out sensitivity analyses (available in supplemented materials) of the effect size aggregates for between-subject studies examining age discrimination of 50- and 60-year-old applicants. These analyses show robust findings generally, with the only exception being the removal of Carlsson and Eriksson (2019), which somewhat increases the effect size and tau. Presumably, the Carlsson and Eriksson (2019) study introduces higher deviation in effects because of the continuous age variable. We tried to account for publication bias by including grey literature, however, it is difficult to assess based on the low number of studies included in the analyses. Following the GRADE guidelines (Higgins et al., 2021), this implies that we are moderately confident in the certainty of our evidence, but we acknowledge the possibility that we did not capture the true effect.
